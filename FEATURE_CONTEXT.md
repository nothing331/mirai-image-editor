...
**Outcome.** An active invited account can reserve space for a PNG/JPEG original, transfer it through a bounded server route, finalize an immutable exact source plus lossless editor base, and obtain a short-lived private read URL. P05 attaches a finalized receipt to a cloud project and provides the user-facing upload flow.
...
**Dependencies and limits.** P03 active accounts, the reviewed P04 migration, Supabase Storage, and the server-only secret key are required. The asset routes run in staging/beta and in local mode when account authentication is enabled; CI mode remains closed. The initial limits are 10 MiB source, 2,048 pixels per edge, 4,194,304 pixels, 30 MiB editor base, 100 MiB per account, and 700 MiB in managed global reservations/assets. These are beta controls, not a promise that arbitrary 10 MiB images will normalize under the base limit. The local editor and local project API remain separate from cloud projects. Cleanup is manually invokable until the P18 maintenance scheduler exists. The Render/Supabase staging walkthrough passed on 2026-09-26.
...
## Cloud project creation and reopening

**Outcome.** An approved account can upload a PNG or JPEG, save it as a named private project, see it in My projects, and reopen the normalized original at `/projects/[id]` after signing out and back in. This is the P05 durable original checkpoint and the entry to the P06/P07 cloud editor.

**Working flow.** `/projects/new` checks type, byte size, name, and the current project count before using the P04 reservation, upload, and finalization routes. After transfer, the browser records the upload ID in account-scoped session storage so finalization or project attachment can be retried after a reload. `POST /api/cloud-projects` calls one Postgres function that locks the owner profile, verifies an active account and a ready owned upload, enforces the five-project cap, and inserts the project plus initial version in one transaction. Retrying with the same upload and name returns the existing project. `/projects` loads only the account's small project metadata list. `/projects/[id]` loads an owned row, signs a 60-second read URL for its normalized base, and renders the original; reopening signs a fresh URL.

**Ownership and rules.** The database owns the project/version relationship, current-version pointer, owner constraints, project cap, and atomic attach. The initial version is insert-only for the service role. Browser roles cannot read or mutate either table directly. The server checks active eligibility on every project route and filters all reads by owner ID; mutations also verify the request origin. The signed image URL is short-lived, and metadata/image responses are private and uncached. P05's cloud API is separate from the local SQLite `/api/projects` path, which remains closed in staging and beta.

**Failures and recovery.** An unfinished or foreign upload cannot create a project. A failed database attach creates no partial project/version. Duplicate attach creates neither duplicate project nor duplicate version. The browser can retry finalization or attachment with the same upload ID from the same account and tab session. Aborted pre-finalization transfers can be cancelled; otherwise P04 cleanup handles expired reservations. A finalized but unattached upload can still consume storage allowance until later cleanup work; the client checks project count before reserving to avoid the ordinary five-project case. If the private image cannot be signed, reopening fails without exposing the storage key.

**Dependencies and limits.** P03 authentication and P04 private assets are required. The first list is intentionally small: no thumbnails, search, sort controls, rename, or deletion until P08/P11. The five active projects per account limit is an initial beta control.
...
## Cloud edit acceptance and history

**Outcome.** An approved project owner can make deterministic edits in the browser, keep the proposed pixels visible until the save succeeds, reopen the same accepted image, and move through saved versions with undo, redo, original, and paged history. P06 and P07 share one delivery because acceptance and navigation depend on the same immutable version chain.

**Working flow.** `/projects/[id]` opens the owned project's original and current PNG, then reuses the local canvas and direct-edit inspectors. The browser stages an accepted proposal with its source-sized mask and one operation; it sends a stable request key and output asset ID to `POST /api/cloud-projects/[id]/edits`. The server validates the current input, local operation, image dimensions, PNG bytes, mask dimensions, and unchanged pixels outside a same-size effective mask. It reserves account/global storage, writes a private immutable PNG, then calls a PostgreSQL transaction that inserts one operation, one version, and a retry receipt while advancing current and head pointers. Only after receiving that receipt does the browser add the edit to its in-memory accepted state. A failed or lost response keeps the proposed image visible for retry with the same keys. A confirmed undo/redo/original/history choice loads only the selected image and moves the durable current pointer. New edits after undo require explicit confirmation to replace the redo future.

**Ownership and rules.** Supabase tables own version sequence, active linear chain, operation metadata, compressed source-sized mask, current/head pointers, quotas, and idempotency receipts. Supabase Storage owns private PNG assets. Browser roles cannot read or mutate these tables; active project ownership is checked on every route, and mutating routes check origin. The server uses a privileged client behind this boundary. The existing editor store owns transient proposals and invokes its shared acceptance pipeline after the database receipt. Original upload bytes and normalized base are never overwritten. The initial history response contains at most 20 metadata entries; opening a project decodes only the original and selected current image, and historical pixels load on demand.

**Failures and recovery.** An invalid image/mask or stale input creates no accepted version. A repeated request with the same payload returns the prior receipt; a reused key with different content fails. An uploaded asset whose database commit does not finish remains charged and can be retried while the tab retains the proposal. Missing current assets surface an error instead of resetting history. Closing the tab with a pending edit prompts the browser's unload warning; durable draft recovery is P09. Detached redo versions stay stored and charged until P18 cleanup, preventing silent deletion during the acceptance transaction.

**Dependencies and limits.** P03–P05, the P06/P07 migration, private Storage, and the service key are required. Cloud mode currently enables deterministic local operations (selection recolor/paint, geometry, text and watermark, and monochrome); AI actions stay disabled until their later cloud delivery. Output remains a source-size or deliberately resized PNG within the 2,048-edge/4,194,304-pixel envelope and a 30 MiB file bound. The accepted watermark pixels survive reopening, but a PNG watermark's separate source overlay is currently only in the active browser session; reopening does not rehydrate that source for later adjustment. The service validates saved pixels and masks, but does not replay browser font rendering. P08/P09/P10/P18 add library controls, durable drafts, complete export controls, and orphan cleanup.

**Code and verification.** `supabase/migrations/20260926120637_cloud_edit_history.sql`, `supabase/tests/cloud_edit_history_test.sql`, `src/server/cloud-projects/cloud-edit-history.ts`, `src/server/cloud-projects/cloud-edit-history.integration.test.ts`, `src/app/api/cloud-projects/[id]/`, `src/features/cloud-projects/CloudEditorWorkspace.tsx`, `src/features/cloud-projects/cloud-edit-client.ts`, `src/features/editor/store.ts`, `src/features/editor/store.test.ts`, `e2e/cloud-editor.spec.ts`, and `docs/cloud/WAVE_B_P06_P07_RUNBOOK.md`.
...
**Limitations.** This local editor flow still stores projects on local-development infrastructure. The separate cloud editor at `/projects/[id]` persists deterministic edits and navigation through Supabase; the local SQLite editor remains local-only.
