...
export async function GET(request: NextRequest) {
  ...
  let response = redirectResponse(request, "/sign-in?error=callback");
  ...
  const supabase = createServerClient(url, publishableKey, {
    cookies: {
      ...
      setAll: (cookiesToSet, cacheHeaders) => {
        ...
        response = redirectResponse(request, "/sign-in?error=callback");
  ...
  try {
    ...
    const destination = account?.profile.status === "active"
      ...
      : returnPath;
    const successfulResponse = redirectResponse(request, destination);
    ...
    return redirectResponse(request, "/sign-in?error=setup");
  }
}

function redirectResponse(request: NextRequest, pathname: string): NextResponse {
